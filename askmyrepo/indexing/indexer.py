...
class Indexer:
    ...
    def index(
        ...
        try:
            ...
            code_chunks = self.chunker.chunk_from_nodes(all_nodes)
            text_chunks = self._chunk_raw_files(repo_path, all_nodes)
    ...
    def _chunk_raw_files(
        ...
        """Create text-based chunks for files that had no AST nodes (edge case coverage)."""
        chunked_paths = {n.file_path for n in parsed_nodes}
        text_chunks: list = []
        for path_str in chunked_paths:
            file_path = repo_root / path_str
            if file_path.exists():
                text_chunks.extend(self.chunker.chunk_raw_text(file_path, repo_root))
        return text_chunks
